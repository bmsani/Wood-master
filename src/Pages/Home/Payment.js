...
const Payment = () => {
    const { id } = useParams();
    const { data: singleOrder, isLoading, refetch } = useQuery('singleOrder', () => fetch(`https://quiet-chamber-70480.herokuapp.com/order/${id}`, {
        headers: {
                    ...
                    <div class="card w-96 bg-base-100 shadow-xl">
                        <div class="card-body">
                            <Elements stripe={stripePromise}>
                                <CheckOutForm singleOrder={singleOrder}></CheckOutForm>
                            </Elements>
                        </div>
                    </div>
                </div>
